...
public class UserController {
    ...
    @PostMapping("/cancel")
    public BaseResponse<Integer> userCancel(@RequestBody User user,HttpServletRequest request) {
        if (request==null&&user==null){
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        User loginUser = userService.getLoginUser(request);
        int result=userService.userCancel(user,loginUser,request);
        return ResultUtils.success(result);
    }

    @PostMapping("/currentbyid")
    public BaseResponse<User>getCurrentUserById(@RequestBody User user,HttpServletRequest request){
